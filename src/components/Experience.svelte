...
Documented and tested codebase before releasing to the company.`,
  ...
  let focusedExperience = experiences[0];
</script>
  ...
  <div class="flex gap-12">
    <div class="w-2/3">
      {#each experiences as experience, index}
        <button
          class={`flex items-center gap-4 w-full text-left text-sm p-2 my-4 rounded-xl hover:bg-secondary ${ focusedExperience === experience ? "text-primary" : "" }`}
          on:click={() => focusedExperience = experiences[index]}
        >
          <img
    ...
    <div class="w-1/2">
        <div class="flex items-end gap-4 mb-4">
            <img src={focusedExperience.employerImage} class="rounded-xl w-24" alt={`${focusedExperience.employer} Logo`}>
            <div>
                <p class="font-semibold text-xl">{ focusedExperience.title }</p>
                <p class="my-1">
                    { focusedExperience.start } -
                    { #if focusedExperience.end}
                        <span>{focusedExperience.end}</span>
                        {:else}
                        <span>Present</span>
                    {/if }
                    
                </p>
                <p>{ focusedExperience.location }</p>
            </div>
        </div>
        <div class="whitespace-pre-line">
            { focusedExperience.description }
        </div>
    </div>
  </div>
</main>
